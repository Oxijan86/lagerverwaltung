...
function adoptExistingDatabase(){
 if(existingDatabaseHasContent()&&setting('setup_complete','0')!=='1'){
  ...
  setSetting('database_version','36');
  if(!setting('date_format'))setSetting('date_format','DD.MM.YYYY');
 }
}
...
async function route(url,opt={}){
 ...
 try{
 if(opt.method!=='POST'){
  if(p==='/api/info')return response({version:'36.0',articles:scalar('SELECT COUNT(*) FROM articles WHERE active=1'),movements:scalar('SELECT COUNT(*) FROM movements'),setup_required:setupIsRequired(),date_format:setting('date_format','DD.MM.YYYY')});
