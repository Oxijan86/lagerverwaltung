...
function adoptExistingDatabase(){
 ...
 if(existingDatabaseHasContent()&&setting('setup_complete','0')!=='1'){
  setSetting('setup_complete','1');setSetting('database_adopted','1');setSetting('database_version','62');
...
async function initialize(){
 ...
 if(setting('database_version','')!=='62'){setSetting('database_version','62');migrated=true}
...
async function route(url,opt={}){
 ...
 try{
 if(opt.method!=='POST'){
  if(p==='/api/info')return response({version:'62.0',articles:scalar('SELECT COUNT(*) FROM articles WHERE active=1'),movements:scalar('SELECT COUNT(*) FROM movements'),setup_required:setupIsRequired(),date_format:setting('date_format','DD.MM.YYYY')});
...
window.LVStartupState={
 ...
 async createNew(){
  ...
  setSetting('database_version','62');
  const m=await ig(METAKEY)||{};
  m.dirty=true;
