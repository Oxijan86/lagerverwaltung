(function(root,factory){
 ...
 async function hashPassword(password){if(String(password).length<8)throw Error('Passwort muss mindestens 8 Zeichen haben.');const c=cryptoObj();const salt=c.getRandomValues(new Uint8Array(16));const hash=await derive(password,salt);return `pbkdf2$${ITERATIONS}$${bytesToB64(salt)}$${bytesToB64(hash)}`}
 ...
 return {ITERATIONS,hashPassword,verifyPassword,createSessionManager};
});
